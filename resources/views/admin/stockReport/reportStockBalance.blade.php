          ...
          <table width="100%" border="1px" style="border: 1px solid gray; border-collapse: collapse;" cellpadding="5px" cellspacing="0">
            <thead>
              <tr>
                ...
                <th colspan="2" style="text-align: center;font-size: 11px;font-weight: bold; padding: 2px 5px; font-family: 'Arial';">Begining Stock</th>
                <th colspan="2" style="text-align: center;font-size: 11px;font-weight: bold; padding: 2px 5px; font-family: 'Arial';">Stock Out</th>
                <th colspan="2" style="text-align: center;font-size: 11px;font-weight: bold; padding: 2px 5px; font-family: 'Arial';">Current Stock</th>
              </tr>
              <tr>
            ...
            <tbody>
              @foreach($productBalance as $balance)
                <?php 
                  $product_id =0;
                  $qtyHis =0;
                  $histories = DB::table('histories')->selectRaw('product_id, sum(qty) as Qty')->groupBy('product_id')->where('product_id','=',$balance->id)->get();
                  $histories = DB::table('histories')->selectRaw('product_id, sum(qty) as Qty')->groupBy('product_id')->where('product_id','=',$balance->id)->get();
                ...
                <tr>
                  ...
                  <td style="text-align: center;font-size: 10px; height: 20px; font-family: 'Arial';">{{$balance->productCode}}</td>
                  <td style="text-align: center;font-size: 10px;height: 20px; font-family: 'Arial';">{{$balance->productBarcode}}</td>
                  <td style="padding-left: 3px; font-size: 10px;height: 20px; font-family: 'Khmer OS System';">{{$balance->khName}}</td>
